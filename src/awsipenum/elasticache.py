...
def elasticache_ips(p: str, r: str):
    ...
    try:
        ...
        result = result_full.get('ReplicationGroups')
    except botocore.errorfactory.ClientError:
        result = False

    if result:
        msg.hdr("Enumerating Elasticache IPs ..")
        for elasticache in result:
            ip_list = []
            i = finding()
            i.id = elasticache['ConfigurationEndpoint']['Address']
            i.region = r
            i.profile = p
            i.name = elasticache['ReplicationGroupId']
